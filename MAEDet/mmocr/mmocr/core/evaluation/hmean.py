...
def get_gt_masks(ann_infos):
    ...
    for ann_info in ann_infos:
        ...
        for mask in masks:
            ...
            try:
                assert len(mask[0]) >= 6 and len(mask[0]) % 2 == 0
            except:
                print(len(mask[0]))
        ...
        for mask_ignore in masks_ignore:
            # if len(mask_ignore[0]) < 8 or len(mask_ignore[0]) % 2 !=0:
            #     print()
            #     print("=".center(100, "="))
            #     print(ann_info['seg_map'], mask_ignore[0])
            # modified by ctj: 3 points can also form a polygon
            # assert len(mask_ignore[0]) >= 8 and len(mask_ignore[0]) % 2 == 0
            assert len(mask_ignore[0]) >= 6 and len(mask_ignore[0]) % 2 == 0
