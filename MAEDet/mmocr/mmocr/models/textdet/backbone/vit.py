...
class VisionTransformer(timm.models.vision_transformer.VisionTransformer):
    ...
    def forward(self, x):
        x = self.forward_features(x)  # (bs, len+1, dim)
        # x = self.unpatchify(x[:, 1:, :])
        bs, seq_len, dim = x[:, 1:, :].shape
        x = x[:, 1:, :].permute(0, 2, 1).reshape(bs, dim, int(math.sqrt(seq_len)), int(math.sqrt(seq_len))).contiguous()
        return x
...
class VisionTransformer_LoRA(nn.Module):
    ...
    def __init__(self,
        ...
        self.vit = build_backbone(vit_config)
        assert rank > 0
        if lora_layers:
